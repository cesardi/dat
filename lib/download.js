...
module.exports = function (type, opts, dat) {
  ...
  function start (err, theDat) {
    ...
    var niceType = (type === 'clone') ? 'Cloning to' : type.charAt(0).toUpperCase() + type.slice(1) + 'ing'
    var matchkey = /\/?([^/]{64})/
    var keylessPath = dat.path.replace(matchkey, '/<dat-key>') // replace key in path for display
    progressOutput[0] = `${niceType} ${keylessPath}`
    ...
    network.swarm.once('connection', function (peer) {
      ...
      onceConnected = true
      progressOutput[1] = 'Starting Download...'
    })
    progressOutput[1] = 'Looking for Dat Archive in Network'
  ...
  function updateDownload () {
    if (!archive.content) {
      progressOutput[1] = '... Fetching Metadata'
      return
    }
    ...
    progressOutput[1] = 'Metadata: ' + bar(metaProgress) + ' ' + metaProgress + '%'
    progressOutput[2] = 'Content:  ' + bar(contentProgress) + ' ' + contentProgress + '%'

    if (!onceConnected && dat.live) progressOutput[3] = 'Waiting for connections to check for updates.'
