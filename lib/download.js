...
module.exports = function (type, opts, dat) {
  ...
  function start (err, theDat) {
    ...
    network = dat.joinNetwork(opts, function (err) {
      if (err) debug('joinNetwork() error', err) // TODO: what errors show up here?
      debug('joinNetwork() callback')
      if (network.connected) return // found a peer
      output[0] = ['No peers currently online, exiting. Run with option TBD to wait for peers.']
      output[1] = []
      log.print()
      exit()
    })
