...
module.exports = function sync (type, opts, dat) {
  ...
  function start (err, theDat) {
    ...
    progressOutput[0] = `Syncing: ${dat.path}\n`
    ...
    if (dat.owner && opts.import) {
      ...
      // File Imports
      progressOutput[1] = 'Importing files...'
      ...
      importStatus = dat.importFiles({
        ...
        var st = stats.get()
        importDone = true
        progressOutput[1] = opts.watch ? 'Watching for file changes...' : 'Archive update finished! Sharing latest files.'
        progressOutput[2] = `Total Size: ${st.filesTotal} ${st.filesTotal === 1 ? 'file' : 'files'} (${prettyBytes(st.bytesTotal)})`
